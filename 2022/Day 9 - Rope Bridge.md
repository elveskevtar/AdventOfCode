...
def simulate_motion(movements):
    head_location = tail_location = (0, 0)
    tail_visited = {tail_location}
    for movement in movements:
        direction, spaces = movement.split()
        spaces = int(spaces)
        delta_move = get_delta(direction)
        for _ in range(spaces):
            head_location = add_coords(head_location, delta_move)
            if not is_touching(head_location, tail_location):
                tail_location = new_tail_location(head_location, tail_location)
